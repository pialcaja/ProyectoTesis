...
import com.ecapi.model.TransaccionSpecification;
...
public class TarjetaBusServiceImpl implements TarjetaBusService {
	...
	public List<MedioPagoDTO> obtenerMediosPago(String email) {
		...
		return mediosPago.stream().map(this::convertMedioPagoToDTO).collect(Collectors.toList());
	}
	...
	public RecargaResponseDTO recargarTarjeta(String email, RecargaRequest request) {
		...
		response.setFechaTransaccion(LocalDate.now());
		...
		try {
			...
			// Simular procesamiento de pago (en producción aquí iría la integración con
			// pasarela)
	...
	private void registrarTransaccion(Usuario usuario, TarjetaBus tarjeta, MedioPago medioPago, double monto,
			String estado) {
	...
	@Override
	public ResponseEntity<Map<String, Object>> listarTransacciones(int page, int size, String estado,
		...
		try {
			...
			Specification<Transaccion> spec = Specification.allOf(TransaccionSpecification.estadoEquals(estado),
					TransaccionSpecification.fechaDesde(fechaDesde), TransaccionSpecification.fechaHasta(fechaHasta));

			Page<Transaccion> transacciones = transaccionRepo.findAll(spec, pageable);
			...
			response.put("mensaje", "Error al listar transacciones: " + e.getMessage());
